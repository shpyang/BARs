FUN.BA=
function(data, outcome.name, outcome.before, outcome.after, id.name, trt.name, trt.levels, covariates, rd, rd2, rd3, LTransformed)
{ if (missing(LTransformed)) {LTransformed=FALSE} 
  if (missing(rd)) {rd=3}
  if (missing(rd2)) {rd2=3}
  if (missing(rd2)) {rd3=3}
  ...
  modf=formula(paste0(" y ~ trt + time + trt:time + base + (1| id)+", covariates_formula))
  m3x <- lmer(modf, REML=TRUE, data=mg3) 
  #m3x <- lmer(y ~ trt + time + trt:time + (1| part_id) +base, mg3)
  mout=summary(m3x)
  anova(m3x) 
  
  # The beflow was not used:
  ...
  #mdout=paste0(round(mdiff[,2],2), " (", round(mdiff[,2]-qt(0.975, mout[[10]][row.names(mout[[10]])=="time", "df"])*mdiff[,3],2),
  #             ", ",  round(mdiff[,2]+qt(0.975, mout[[10]][row.names(mout[[10]])=="time", "df"])*mdiff[,3],2),")")
  
  ########################
  # The below was used, which has more similar results as sas:
  #proc mixed data=data method=REML;;
  #class trt sex id time;
  #model y=trt time trt*time base sex/ cl ddfm=kr;;
  #random id;
  #lsmeans trt trt*time/pdiff cl;
  #run;
  
  em2=as.data.frame(emmeans(m3x, pairwise ~ trt * time, adjust = "tukey")[[2]])
  mdiff=as.data.frame(rbind(em2[em2$contrast==paste0(trt.levels[1], " time0 - ", trt.levels[1], " time1"),],
                            em2[em2$contrast==paste0(trt.levels[2], " time0 - ", trt.levels[2], " time1"),]))
  
  mdout=paste0(round(-mdiff[,2],rd2), "(", round(-mdiff[,2]-qt(0.975, mout[[10]][row.names(mout[[10]])=="time", "df"])*mdiff[,3],rd2),
               ", ",  round(-mdiff[,2]+qt(0.975, mout[[10]][row.names(mout[[10]])=="time", "df"])*mdiff[,3],rd2),")")
  ...
  ddiff=rbind(t(c("Reference", "")), c(paste0(round(ddifft[1,1],rd2), "(", round((ddifft[1,1]-qt(.975, ddifft[,3])*ddifft[1,2]),rd2), ", ",
                                              round((ddifft[1,1]+qt(.975, ddifft[,3])*ddifft[1,2]),rd2), ")"), round(ddifft[1,5],rd3)))
