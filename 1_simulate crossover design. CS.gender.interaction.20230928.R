...
# To calculate the power for testing treatment x gender interaction is the below
...
This equals to the power for the Treatmentxgender interaction for model
model <- lme(Response ~ Treatment +Period +gender+Treatment*gender, random = ~ 1 | Subject , data = datas)

This also equal to the paired comparison of two treatments between the two groups
genderAB.p[b]=t.test(mg$C_A~mg$gender.x)$p.value

The sd for mg$C_A is 
sqrt((2-2*corr))